...
Replace runtime Bun shell template-tag usage in `packages/core/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/core/src` is removed except:
