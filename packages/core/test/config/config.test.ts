...
const managedConfigDir = process.env.OPENMODS_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["OPENMODS_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:OPENMODS_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["OPENMODS_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENMODS_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads config from .openmods directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openmods")
...
test("loads agents from .openmods/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openmods")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openmods/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openmods")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openmods/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openmods")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only OPENMODS_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENMODS_CONFIG_DIR
  process.env.OPENMODS_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENMODS_CONFIG_DIR
    else process.env.OPENMODS_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENMODS_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openmods config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".openmods")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .openmods config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openmods config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets OPENMODS_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openmods config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openmods directory config enables it
      const opencodeDir = path.join(dir, ".openmods")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.openmods/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openmods/plugin/oh-my-opencode.js")
  })
...
describe("OPENMODS_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENMODS_DISABLE_PROJECT_CONFIG"]
    process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENMODS_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openmods/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENMODS_DISABLE_PROJECT_CONFIG"]
    process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openmods directory with a command
          const opencodeDir = path.join(dir, ".openmods", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .openmods should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENMODS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENMODS_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENMODS_CONFIG_DIR"]
      process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENMODS_CONFIG_DIR"]
      } else {
        process.env["OPENMODS_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENMODS_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENMODS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENMODS_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENMODS_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENMODS_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENMODS_CONFIG_DIR, not project
...
describe("OPENMODS_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENMODS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENMODS_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENMODS_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENMODS_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENMODS_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENMODS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENMODS_CONFIG_CONTENT"]
