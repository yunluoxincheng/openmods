...
afterEach(async () => {
  delete process.env.OPENMODS_CONFIG
  delete process.env.OPENMODS_TUI_CONFIG
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".openmods"), { recursive: true })
      await Bun.write(
        path.join(dir, ".openmods", "tui.json"),
...
test("project config takes precedence over OPENMODS_TUI_CONFIG (matches OPENMODS_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.OPENMODS_TUI_CONFIG = custom
    },
  })
...
test("OPENMODS_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from OPENMODS_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.OPENMODS_CONFIG = path.join(customDir, "opencode.json")
    },
  })
...
test("loads .openmods/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".openmods"), { recursive: true })
      await Bun.write(path.join(dir, ".openmods", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
