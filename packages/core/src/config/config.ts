...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/openmods"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "openmods")
      default:
        return "/etc/openmods"
    }
  }
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${url}/.well-known/openmods` })
        const response = await fetch(`${url}/.well-known/openmods`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://openmods.dev/config.json"
        result = mergeConfigConcatArrays(
          result,
          await load(JSON.stringify(remoteConfig), {
            dir: path.dirname(`${url}/.well-known/openmods`),
            source: `${url}/.well-known/openmods`,
          }),
        )
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.openmods/command/", "/.openmods/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.openmods/agent/", "/.openmods/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://openmods.dev/docs/commands"),
      ...
      default_agent: z
        ...
        .optional()
        .describe(
          "Default agent to use when none is specified. Must be a primary agent. Falls back to 'minecraft' if not set or if the specified agent is invalid.",
        ),
      username: z
        .string()
        .optional()
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://openmods.dev/docs/agents"),
      provider: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "openmods.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "openmods.jsonc"))),
    )
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://openmods.dev/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://openmods.dev/config.json"
        const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://openmods.dev/config.json",')
