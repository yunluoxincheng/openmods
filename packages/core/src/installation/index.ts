...
declare global {
  const OPENMODS_VERSION: string
  const OPENMODS_CHANNEL: string
}
...
export namespace Installation {
  ...
  async function upgradeCurl(target: string) {
    const body = await fetch("https://openmods.dev/install").then((res) => {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".openmods", "bin"))) return "curl"
    ...
    const checks = [
      ...
      {
        name: "brew" as const,
        command: () => text(["brew", "list", "--formula", "openmods"]),
      },
      {
        name: "scoop" as const,
        command: () => text(["scoop", "list", "openmods"]),
      },
      {
        name: "choco" as const,
        command: () => text(["choco", "list", "--limit-output", "openmods"]),
      },
    ]
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "openmods" : "openmods-ai"
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch("https://api.github.com/repos/openmods-ai/openmods/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
