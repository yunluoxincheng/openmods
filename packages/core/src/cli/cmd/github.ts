          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/om summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://openmods.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
...
jobs:
  openmods:
    if: |
      ...
      startsWith(github.event.comment.body, '/om') ||
      contains(github.event.comment.body, ' /openmods') ||
      startsWith(github.event.comment.body, '/openmods')
    ...
    steps:
      ...
      - name: Run openmods
        uses: openmods-ai/openmods/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.openmods.dev" : "https://openmods.dev"
      ...
      async function chat(message: string, files: PromptFiles = []) {
        ...
        const result = await SessionPrompt.prompt({
          ...
          // agent is omitted - server will use default_agent from config or fall back to "minecraft"
          parts: [
            {
              id: PartID.ascending(),
              type: "text",
