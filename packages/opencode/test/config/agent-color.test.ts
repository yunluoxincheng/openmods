...
test("agent color parsed from project config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(cfg.agent?.["minecraft"]?.color).toBe("#FFA500")
...
test("Agent.get includes color from config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const minecraft = await AgentSvc.get("minecraft")
      expect(minecraft?.color).toBe("accent")
    },
  })
})
