...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@openmods-ai/sdk/v2"
...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENMODS_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.OPENMODS_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.OPENMODS_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.OPENMODS_SERVER_USERNAME ?? "opencode"
