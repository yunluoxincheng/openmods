...
const keepSandbox = process.env.OPENMODS_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  OPENMODS_DISABLE_SHARE: process.env.OPENMODS_DISABLE_SHARE ?? "true",
  OPENMODS_DISABLE_LSP_DOWNLOAD: "true",
  OPENMODS_DISABLE_DEFAULT_PLUGINS: "true",
  OPENMODS_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  OPENMODS_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  OPENMODS_E2E_PROJECT_DIR: repoDir,
  OPENMODS_E2E_SESSION_TITLE: "E2E Session",
  OPENMODS_E2E_MESSAGE: "Seeded for UI e2e",
  OPENMODS_E2E_MODEL: "opencode/gpt-5-nano",
  OPENMODS_CLIENT: "app",
  OPENMODS_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_OPENMODS_SERVER_HOST: "127.0.0.1",
  VITE_OPENMODS_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    process.env.OPENMODS_PID = String(process.pid)
